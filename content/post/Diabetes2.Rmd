...
Out paper is designed to look at the effects of work and lifestyle on the age at which the participants of the General Social Survey (GSS) on health, are diagnosed with diabetes. Our model will look at a select few variables and attempt to create regression models based on these variables. Our study has concluded that 
...
```{r read data, include = FALSE, error = FALSE}
...
```{r select variable}
health <- raw_health %>% select(CASEID, dvagegr, dvsex, dvperinc, dvbmimdf, a9cap, g2cap, g3cap, h2hrs, k4, m17cap) 
...
lmhealth <- lm(age_first_diagnosed_with_diabetes ~ sleep_hours + work_hours_per_week
               + income_12months, data=health_linear)
```
